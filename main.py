...
cursor.execute('''CREATE TABLE IF NOT EXISTS bot(model TEXT DEFAULT 'text' , tag TEXT DEFAULT 'off', setid TEXT , setgp TEXT , texttag TEXT , repcht INTEGER , repmes INTEGER ,  time INTEGER DEFAULT '1')''')
...
async def spamer_handler(client, message):
    ...
    if str(Userid) in listadmin:
        if text == "bot":
            await message.reply_text('ربات اتکر علیرضا ولف آنلاین هست' , quote = True)

        if text == "start":
            await message.reply_text('Ok.!' , quote = True)
            
        if text == "داش":
            await message.reply_text('داشمی ❤' , quote = True)
    ...
    ❋ owner ⤳  <b>Coder By Alireza Wolf</b>  🧑‍💻
            """)
    ...
    ❋ `addfosh` ⤳ اضافه کردن فوش مورد نظر
    ...
    ❋ owner ⤳ @PvGoRg 🧑‍💻
            ...
            await message.reply_text('لطفا فوش بعد `addfosh` بفرستید \n مثال : addfosh سلام')
